...
通过 CLI 获取受控 Query Packet，再进行语义回答。查询不修改 Raw、Wiki 或 Schema；默认仅在 `.lore/usage/queries/` 记录隐私安全的召回统计。
...
3. 将用户的真实问题原样传给：
   ...
   返回的 `query_id` 用于本地 Dashboard 聚合；默认记录问题哈希而非问题原文。用户明确要求本次不留召回统计时增加 `--no-track`。
...
- 召回统计只表示候选进入 Query Packet，不得把它表述为 Agent 最终引用或用户认可。
